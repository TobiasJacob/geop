...
- **Splitting the code into geometry and topology**: At first, the code was split into geometry and topology. Geometry dealt with infinite lines, circles, etc. Topology dealt with finite edges, faces, volumes. This worked well for a while, but then we realized that this was a bad idea. Intersections for example would be implemented in a way where the geometry was intersected first, yielding a fininte, infinite number (in case of helix-line intersection) or a curve as intersection. Then this was narrowed down, due to the containment condition. This was a lot more cumbersom than just working with the geometry directly. Now a linear edge for example is defined by its start and end point, and not start, end, anchor and direction.
